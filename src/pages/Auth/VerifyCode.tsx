...
export default function VerifyCode() {
  ...
  const { pendingEmail, error, isLoading, verifyResetCode, resendOtp } = useAuthStore();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      sessionStorage.setItem("resetOtp", verificationCode); // keep OTP for reset step
          ...
          <div className="flex justify-center gap-4 w-full">
            {code.map((digit, index) => (
              <input
                ...
                onPaste={handlePaste}
                className="min-w-12 aspect-square text-2xl text-center border border-[#D5D5D5] text-[#101010] rounded-[5px] focus:outline-none focus:ring-2 focus:ring-[#31BB5E] font-semibold"
              />
            ))}
          </div>
